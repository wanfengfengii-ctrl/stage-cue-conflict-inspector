...
src/
  core/            # 纯逻辑：类型、校验、冲突检测、格式化、JSON 文本定位、
                   #        compactScale（整日/紧凑分段映射）、conflictKey（无碰撞选择身份）、
                   #        overlapHit（交集热区坐标命中解析）（均含单测）
  components/      # InputPanel / Timeline / ConflictList
  App.tsx          # 粘贴即检测；非法输入整批拒绝、不沿用旧结果；编辑 JSON 恢复整日模式
e2e/               # Playwright 端到端测试（含紧凑模式切换、1ms 压缩交集、断轴）
